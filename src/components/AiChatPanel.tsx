    ...
    <div className="flex flex-col h-full bg-slate-900 text-slate-100 relative select-text">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 min-h-0">
        {displayMessages.length === 0 && !isLoading && !streamingText && (
          <div className="flex flex-col items-center justify-center h-full text-center py-10 text-slate-500 space-y-3">
            <div className="w-16 h-16 rounded-2xl bg-slate-800/60 border border-slate-700/50 flex items-center justify-center text-slate-400 text-2xl shadow-inner">
              <i className="fas fa-robot"></i>
            </div>
            <div className="max-w-xs">
              <p className="text-sm font-semibold text-slate-300">How can I assist your operations?</p>
              <p className="text-xs text-slate-400 mt-1">
                Ask about live telemetry, alarms, inspect attached SCADA screenshots, or generate schematics.
              </p>
            </div>
            <div className="flex flex-wrap justify-center gap-2 mt-2 max-w-sm">
              {[
                'What alarms are active?',
                'Summarize all dashboards',
                'Generate a cooling water P&ID diagram',
      ...
      <div className="p-3 border-t border-slate-800/80 bg-slate-900/95 shrink-0">
        ...
        {isQuotaLocked && (
          <div className="mb-2.5 p-3.5 bg-amber-950/60 border border-amber-500/50 rounded-2xl flex items-center justify-between space-x-3 shadow-xl backdrop-blur-md animate-fadeIn">
            <div className="flex items-center space-x-3">
              <div className="w-10 h-10 rounded-xl bg-amber-500/20 border border-amber-500/40 flex items-center justify-center text-amber-400 text-lg shrink-0 shadow-inner">
                <i className="fas fa-lock"></i>
              </div>
              <div>
                <div className="text-xs font-black text-amber-200 uppercase tracking-wide flex items-center space-x-2">
                  <span>Community Daily Quota Reached</span>
                  <span className="bg-amber-500/25 text-amber-300 text-[9px] px-1.5 py-0.2 rounded font-mono font-bold border border-amber-500/40">5/5 USED</span>
                </div>
                <div className="text-[11px] text-slate-300 mt-0.5 leading-snug">
                  Your 24-hour quota resets in <span className="font-mono font-bold text-amber-300 bg-amber-950/80 px-1.5 py-0.2 rounded border border-amber-500/30">{quotaStatus?.formattedTimeUntilReset}</span>. Upgrade to Engineering Studio for unlimited prompts.
                </div>
              </div>
            </div>
            <div className="hidden sm:flex flex-col items-end text-right">
              <span className="text-[9px] font-bold text-amber-400 uppercase tracking-wider">Engineering Edition</span>
              <span className="text-[10px] text-slate-400 font-medium">Unlimited AI Prompts</span>
            </div>
          </div>
        )}
